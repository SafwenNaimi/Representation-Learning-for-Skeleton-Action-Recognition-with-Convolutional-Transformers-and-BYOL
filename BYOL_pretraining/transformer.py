...
class PatchClassEmbedding(tf.keras.layers.Layer):
    def __init__(self, d_model, pos_emb=None, kernel_initializer='he_normal', **kwargs):
        super(PatchClassEmbedding, self).__init__(**kwargs)
        self.d_model = d_model
        #self.n_tot_patches = 13
        ...
        if self.pos_emb is not None:
            ...
            self.position_embedding = tf.keras.layers.Embedding(input_dim=(31), output_dim=self.d_model)
    ...
    def call(self, inputs):
        ...
        if self.pos_emb is None:
            positions = tf.range(start=0, limit=31, delta=1)
