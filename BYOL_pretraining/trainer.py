...
class Trainer:
    ...
    def build_act(self):
        ...
        #print(base_model)
        input = tf.keras.layers.Input(shape=(self.config[self.config['DATASET']]['FRAMES'] // self.config['SUBSAMPLE'], 
                                              self.config[self.config['DATASET']]['KEYPOINTS'] * self.config['CHANNELS']))
        print("this is the input")
        print(input.shape)
        """
        x = tf.keras.layers.Dense(self.d_model)(inputs)
        x = tf.keras.layers.Conv1D(filters=192, kernel_size=1, activation='relu', padding='same')(x)
        x = tf.keras.layers.Conv1D(filters=192, kernel_size=1, activation='relu', padding='same')(x)
        ...
        """
        intermediate_layer = 12 ##9
        x = base_model.layers[intermediate_layer].output
        #print(len(base_model()))
        outputs = tf.keras.layers.Dense(10, name='dense1000')(x)
        return tf.keras.models.Model(base_model.inputs, outputs)
    


    def get_model(self):
        #transformer = TransformerEncoder(192, 4, 768, 0.3, tf.nn.gelu, 6)
        self.model = self.build_act()
        print(self.model)
