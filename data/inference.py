...
def write_keypoints_to_file(keypoints_list, file_path):
    with open(file_path, 'w') as f:
        json.dump(keypoints_list, f)

def calculate_angle(keypoints, start, mid, end):
    start_point = keypoints[start]
    mid_point = keypoints[mid]
    end_point = keypoints[end]

    vector1 = (mid_point[0] - start_point[0], mid_point[1] - start_point[1])
    vector2 = (end_point[0] - mid_point[0], end_point[1] - mid_point[1])
...
#left_arm_angle = calculate_angle(keypoints, skeleton, 6, 8, 10)
...
if __name__ == "__main__":
    ...
    try:  # Check if is webcam
        ...
        is_video = input_path[input_path.rfind('.') + 1:].lower() in ['mp4', 'mov', 'avi']
    ...
    for (ith, img) in tqdm.tqdm(enumerate(reader), total=total_frames):
        ...
        for frame_number, keypoints_data in frame_keypoints.items():
            keypoints = keypoints_data  # Extract only the x, y coordinates
            #print(keypoints)
            keypoints_permuted = np.copy(keypoints)
            keypoints_permuted[6]= keypoints[14]
            keypoints_permuted[7]= keypoints[6]
            keypoints_permuted[9]= keypoints[10]
            keypoints_permuted[10]= keypoints[7]
            keypoints_permuted[11]= keypoints[9]
            keypoints_permuted[12]= keypoints[11]
            keypoints_permuted[19]= keypoints[12]
            keypoints_permuted[20]= keypoints[15]
            keypoints_permuted[21]= keypoints[17]
            keypoints_permuted[22]= keypoints[21]
            keypoints_permuted[23]= keypoints[19]
            keypoints_permuted[24]= keypoints[20]
            keypoints_permuted[14]= keypoints[16]
            keypoints_permuted[15]= keypoints[18]
            keypoints_permuted[16]= keypoints[24]
            keypoints_permuted[17]= keypoints[22]
            keypoints_permuted[18]= keypoints[23]

            #print(keypoints_permuted)
            keypoints_flat = keypoints_permuted.flatten().tolist()

            keypoints_formatted = "[" + ", ".join(str(value) for value in keypoints_flat) + "]"
            ...
            if len(all_keypoints) >= 30:
                ...
                if len(last_block) < 30:
                    # Remove the last incomplete block
                    keypoints_blocks = keypoints_blocks[:-1]
